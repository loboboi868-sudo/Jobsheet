...
namespace Characteredemo
{
    ...
    public Player(string id, string name, string charClass)
     ...
     public Player(string id, string name, string charClass, int initialHealth, int ILevel)
        {
            CharacterId = id;
            characterName = name;
            characterClass = charClass;
            ...
            level = ILevel;
            playerCount++;
            Console.WriteLine($"Objek dibuat dengan konstruktor berparameter (id, nama, kelas, HP, level).");
        }
    //Method=-
    public void Start()
        {
